...
def run():
    ...
    args_10: tuple = ("--b-channel", "-bc")
    args_11: str  = "--size"
    args_12: str  = "--augment" 
    args_13: str  = "--patience-eps"
    args_14: str  = "--test"
    args_15: tuple = ("--kaggle", "-k")
    ...
    if args_10[0] or args_10[1] in sys.argv: b_channel = True

    if args_11 in sys.argv: size = int(sys.argv[sys.argv.index(args_11) + 1])

    if args_12 in sys.argv: augment = True

    if args_13 in sys.argv:
        use_scheduler = True
        patience = int(sys.argv[sys.argv.index(args_13) + 1])
        eps = float(sys.argv[sys.argv.index(args_13) + 2])
    
    if args_14 in sys.argv: 
        test = True
        name = sys.argv[sys.argv.index(args_14) + 1]
    
    if args_15[0] in sys.argv or args_15[1] in sys.argv: kaggle = True
    ...
    if not test:
        ...
        dataloaders = prepare_train_and_valid_dataloaders(path, mode, b_channel, batch_size, seed, augment)
        ...
        L, _, _ = fit(model=model, optimizer=optimizer, scheduler=scheduler, 
